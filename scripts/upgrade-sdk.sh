...
sed -e "s/ARG sdk_vsn=.*/ARG sdk_vsn=$SDK_VERSION/" -i "${PROJECT_DIR}/Dockerfile-navigator"

echo "modifying ${PROJECT_DIR}/Dockerfile-sandbox"
sed -e "s/ARG sdk_vsn=.*/ARG sdk_vsn=$SDK_VERSION/" -i "${PROJECT_DIR}/Dockerfile-sandbox"
